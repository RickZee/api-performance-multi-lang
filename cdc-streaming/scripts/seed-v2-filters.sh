#!/usr/bin/env bash
# Seed V2 filters into Metadata Service database
# This script reads filters-v2.json and creates them via Metadata Service API
...
FILTERS_FILE="${FILTERS_FILE:-./config/filters-v2.json}"
SCHEMA_VERSION="${SCHEMA_VERSION:-v2}"
...
echo "Seeding V2 filters from $FILTERS_FILE to Metadata Service at $METADATA_SERVICE_URL"
...
# Parse filters-v2.json and create each filter
...
if command -v jq &> /dev/null; then
    ...
    echo ""
    echo "V2 filter seeding complete!"
