...
The filter configuration system is designed to work with any CI/CD platform. Here are examples for common platforms:
...
jobs:
  ...
  deploy:
    ...
    steps:
      ...
      - name: Generate filters
        run: ./cdc-streaming/scripts/filters/generate-filters.sh
      ...
      - name: Deploy Spring filters
        run: ./cdc-streaming/scripts/filters/deploy-spring-filters.sh
  ...
  cleanup:
    ...
    steps:
      - uses: actions/checkout@v3
      - name: Cleanup orphaned resources (dry-run)
        env:
          FLINK_COMPUTE_POOL_ID: ${{ secrets.FLINK_COMPUTE_POOL_ID }}
          KAFKA_CLUSTER_ID: ${{ secrets.KAFKA_CLUSTER_ID }}
        run: ./cdc-streaming/scripts/filters/cleanup-filters.sh --list-orphaned --dry-run
```
