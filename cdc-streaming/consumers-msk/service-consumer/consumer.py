...
def process_service_event(event_value: dict) -> None:
    ...
    try:
        ...
        logger.error(f"Error processing service event: {e}", exc_info=True)

def main():
    """Main entry point"""
    consumer = MSKConsumerBase(
        consumer_name='service-consumer-msk',
        default_topic='filtered-service-events-msk',
        default_group_id='service-consumer-group-msk',
        process_event_callback=process_service_event
    )
    consumer.start()
