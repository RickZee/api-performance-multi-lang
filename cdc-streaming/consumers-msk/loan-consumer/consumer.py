...
def process_loan_event(event_value: dict) -> None:
    ...
    try:
        ...
        logger.error(f"Error processing loan event: {e}", exc_info=True)

def main():
    """Main entry point"""
    consumer = MSKConsumerBase(
        consumer_name='loan-consumer-msk',
        default_topic='filtered-loan-created-events-msk',
        default_group_id='loan-consumer-group-msk',
        process_event_callback=process_loan_event
    )
    consumer.start()
