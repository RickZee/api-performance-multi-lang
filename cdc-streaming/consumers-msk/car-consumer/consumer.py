#!/usr/bin/env python3
"""
Car Consumer for MSK
Consumes filtered car created events from MSK topic: filtered-car-created-events-msk
...
logger = logging.getLogger('car-consumer-msk')

def process_car_event(event_value: dict) -> None:
    """Process a car event"""
    try:
        ...
        logger.info("=" * 80)
        logger.info(f"Car Event Received (MSK)")
        ...
        logger.error(f"Error processing car event: {e}", exc_info=True)

def main():
    """Main entry point"""
    consumer = MSKConsumerBase(
        consumer_name='car-consumer-msk',
        default_topic='filtered-car-created-events-msk',
        default_group_id='car-consumer-group-msk',
        process_event_callback=process_car_event
    )
    consumer.start()
