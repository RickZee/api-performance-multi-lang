...
MONITOR_COMMANDS=(
    ...
    "docker logs dsql-kafka-connect 2>&1 | grep -iE 'error|exception|failed|fatal' | tail -20 || echo 'No errors found'"
    "echo ''"
    "echo '=== 7. Recent Logs (last 30 lines) ==='"
    "docker logs --tail 30 dsql-kafka-connect 2>&1 | tail -30"
    "echo ''"
