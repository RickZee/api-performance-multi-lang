...
public class IamTokenGenerator {
    ...
    private String generateNewToken() {
        try {
            ...
            // Pass iamUsername to determine correct Action (DbConnectAdmin for admin, DbConnect for IAM users)
            String queryString = SigV4QueryStringSigner.generatePresignedQueryString(
                    endpoint, port, credentials, region, iamUsername);
            ...
            // Debug logging: output token format for comparison (first 150 chars)
            String tokenPreview = token.length() > 150 ? token.substring(0, 150) + "..." : token;
            LOGGER.info("Generated DSQL presigned URL token (length: {}, preview: {})", token.length(), tokenPreview);
            LOGGER.debug("Full token format: endpoint={}, port={}, action={}", endpoint, port, 
                       "admin".equalsIgnoreCase(iamUsername) ? "DbConnectAdmin" : "DbConnect");
            return token;
