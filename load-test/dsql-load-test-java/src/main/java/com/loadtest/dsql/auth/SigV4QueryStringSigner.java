package com.loadtest.dsql.auth;
...
class SigV4QueryStringSigner {
     ...
     * @param iamUsername IAM database username (determines Action: DbConnectAdmin for admin, DbConnect for IAM users)
    ...
    static String generatePresignedQueryString(String endpoint, int port, 
                                                AwsCredentials credentials, Region region, String iamUsername) {
        try {
            Instant now = Instant.now();
            String dateStamp = DATE_FORMATTER.format(now);
            String timestamp = TIMESTAMP_FORMATTER.format(now);
            
            // Determine action based on username
            // Admin users need DbConnectAdmin, IAM database users need DbConnect
            String action = "admin".equalsIgnoreCase(iamUsername) ? "DbConnectAdmin" : "DbConnect";
            
            // Build query parameters (sorted for canonicalization)
            // IMPORTANT: Session token must be added BEFORE creating canonical request
            // so it's included in the signature calculation
            ...
            // Add session token if present (required for IAM role credentials)
            // Must be added before canonical request creation to be included in signature
            if (credentials instanceof software.amazon.awssdk.auth.credentials.AwsSessionCredentials) {
                ...
                if (sessionCreds.sessionToken() != null && !sessionCreds.sessionToken().isEmpty()) {
                    queryParams.put("X-Amz-Security-Token", sessionCreds.sessionToken());
                }
            }
            
            // Create canonical request (includes session token if present)
