...
# Get final counts
FINAL_COUNT=$(./scripts/query-dsql.sh "SELECT COUNT(*) FROM car_entities_schema.business_events WHERE id LIKE 'load-test-individual-%';" 2>/dev/null | grep -E '^[0-9]+$' | head -1 || echo "0")
