...
func TestFilterE2E_CreateGenerateApproveDeploy(t *testing.T) {
	...
	t.Run("CreateFilter", func(t *testing.T) {
		...
		t.Run("GenerateSQL", func(t *testing.T) {
			...
			sql := sqlResponse.SQL
			if !containsFilterSQL(sql, "CREATE TABLE") {
				t.Error("Expected SQL to contain CREATE TABLE statement")
			}
			if !containsFilterSQL(sql, "INSERT INTO") {
				t.Error("Expected SQL to contain INSERT INTO statement")
			}
			if !containsFilterSQL(sql, "filtered-service-events-dealer-001") {
				t.Error("Expected SQL to contain output topic name")
			}
			if !containsFilterSQL(sql, "CarServiceDone") {
...
// Helper function
func containsFilterSQL(s, substr string) bool {
	return len(s) >= len(substr) && (s == substr ||
		(len(s) > len(substr) && containsSubstringFilter(s, substr)))
}

func containsSubstringFilter(s, substr string) bool {
