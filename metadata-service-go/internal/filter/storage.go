...
func (s *Storage) Create(version string, req *FilterRequest) (*Filter, error) {
	...
	// Check if filter already exists (using internal method to avoid lock contention)
	filtersDir := s.getFiltersDir(version)
...
func (s *Storage) Get(version, filterID string) (*Filter, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.loadFilter(version, filterID)
}

// loadFilter loads a filter from disk (must be called with lock held)
func (s *Storage) loadFilter(version, filterID string) (*Filter, error) {
	filtersDir := s.getFiltersDir(version)
	filterPath := filepath.Join(filtersDir, filterID+".json")
...
func (s *Storage) Update(version, filterID string, req *UpdateFilterRequest) (*Filter, error) {
	...
	// Load filter directly to avoid nested lock
	filter, err := s.loadFilter(version, filterID)
	if err != nil {
		return nil, err
	}
...
func (s *Storage) UpdateDeployment(version, filterID string, statementIDs []string, err error) error {
	...
	// Load filter directly to avoid nested lock
	filter, e := s.loadFilter(version, filterID)
	if e != nil {
		return e
	}
