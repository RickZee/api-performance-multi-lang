...
from models.event import Event, Entity
...
class EventProcessingService:
    ...
    async def process_entity_update(self, entity_update: EntityUpdate, event_id: str,
        ...
        exists = await entity_repo.exists_by_entity_id(entity.entity_header.entity_id, conn=conn)
        
        if exists:
            logger.warning(
                f"{API_NAME} Entity already exists, updating: {entity.entity_header.entity_id}"
            )
            await self.update_existing_entity(entity_repo, entity, event_id, conn=conn)
        else:
            logger.info(
                f"{API_NAME} Entity does not exist, creating new: {entity.entity_header.entity_id}"
            )
