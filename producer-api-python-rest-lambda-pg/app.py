...
from repository import get_connection_pool, close_connection_pool
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        global _pool
        if _pool and hasattr(_pool, '_loop'):
            pool_loop_id = id(_pool._loop)
...
# Global connection pool (for PG version - needed for shutdown)
_pool = None
...
async def get_service() -> EventProcessingService:
    ...
    if _service is None:
        ...
        # #endregion
        global _pool
        _pool = await get_connection_pool(_config.database_url)
        # #region agent log
        try:
            loop_after_pool = asyncio.get_running_loop()
            loop_id_after_pool = id(loop_after_pool)
            with open('/Users/rickzakharov/dev/github/api-performance-multi-lang/.cursor/debug.log', 'a') as f:
                f.write(json.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"A,B","location":"app.py:58","message":"AFTER get_connection_pool","data":{"loop_id":loop_id_after_pool,"pool_id":id(_pool)},"timestamp":int(__import__('time').time()*1000)})+'\n')
        except: pass
        # #endregion
        ...
        # Create connection factory for pool connections (PG pattern)
        async def connection_factory():
            # Acquire connection from pool
            # The shared service will automatically release it back to the pool
        ...
        _service = EventProcessingService(
            ...
            api_name=API_NAME,
            should_close_connection=True  # Service will detect pool connection and release instead of close
        )
