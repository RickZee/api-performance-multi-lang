...
DSQL_ENDPOINT_PRIMARY="$(terraform output -raw aurora_dsql_endpoint 2>/dev/null || true)"
...
if [ -z "$DSQL_ENDPOINT_PRIMARY" ]; then
  echo "Could not read terraform output aurora_dsql_endpoint. Did you run terraform apply?"
  exit 1
fi

cat > "$OUT_FILE" <<EOF
# Generated by terraform/confluent-option-a/setup-connect-env.sh
