...
class TestEventProcessingService:
    ...
    @pytest.mark.asyncio
    async def test_process_event(self, service, mock_connection, sample_event):
        ...
        with patch.object(service, '_get_entity_repository') as mock_get_repo:
            ...
            # Verify business event was saved
            service.business_event_repo.create.assert_called_once()
            # Verify entity was processed
            mock_repo.exists_by_entity_id.assert_called_once()
            mock_repo.create.assert_called_once()
            # Verify connection cleanup (close for direct connections, release for pool)
            # Since mock_connection doesn't have _pool, it should be closed
            if hasattr(mock_connection, '_pool') and mock_connection._pool is not None:
                mock_connection._pool.release.assert_called_once()
            else:
